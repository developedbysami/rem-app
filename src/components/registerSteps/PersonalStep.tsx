...
import { personalSchema } from "@/schemas/authSchema";
import { PersonalStepProps } from "@/types";
import { handleFileChange } from "@/utils/fileHelpers";
...
export default function PersonalStep({
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center mr-4 text-sky-500 border border-slate-100">
            <User size={20} />
          </div>
          <div>
            <h1 className="font-bold text-slate-800 text-md">
              Individual Agent
            </h1>
            <p className="text-sm text-gray-500">
              I'm a solo real estate agent
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col items-center">
        <div className="relative group">
          ...
          <button
            ...
            className="absolute bottom-1 right-1 bg-sky-600 text-white rounded-full p-1.5"
          >
            <Camera size={14} />
          </button>
        </div>
        <input
          ...
          hidden
          onChange={(e) =>
            handleFileChange(e, true, setProfileImage, setCertificate)
          }
        />
        <p className="text-sm text-gray-600">Profile Photo (Optional)</p>
        <span className="text-xs text-gray-500">
          PNG, JPG only up to 10MB. Professional headshot recommended.
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-1.5">
          ...
          <input
            {...register("firstName")}
            className="input p-3 border rounded-lg focus:border-sky-400 outline-none"
  ...
  <div className="relative">
    <select
      {...register("userRole")}
      className="w-full appearance-none p-3 pr-10 border rounded-lg focus:border-sky-400 outline-none bg-white"
    >
  ...
  {errors.userRole && (
    <p className="text-red-500 text-sm mt-1">
        ...
        <div className="space-y-4">
          ...
          {!certificate ? (
            ...
            >
              <Upload
                size={40}
                className="mb-4 text-gray-400 group-hover:text-[#0081C9]"
              />
          ...
          <input
            ...
            hidden
            onChange={(e) =>
              handleFileChange(e, false, setProfileImage, setCertificate)
            }
      ...
      <div className="flex justify-end mt-12 pt-6 border-t border-gray-300">
        <button
          type="submit"
          className="bg-[#0081C9] hover:bg-[#006da8] text-white px-10 py-2.5 rounded-lg font-bold shadow-md transition-all active:scale-95"
        >
          Next →
        </button>
      </div>
