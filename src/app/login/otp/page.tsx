...
function OTPContent() {
  ...
  const onSendOTP = async (data: any) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/api/v1/otp/request", payload);
    ...
    <div className="w-full flex flex-col items-center justify-center min-h-screen bg-slate-50/30 px-4">
      <div className="text-center mb-10">
        <div className="flex justify-center mb-6">
          <img src="/logo.png" alt="REM Logo" className="h-16 w-auto object-contain" />
        </div>
        <div className="flex items-center justify-center gap-3 mb-3">
          <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600 border border-blue-100">
            {isEmail ? <Mail size={24} strokeWidth={2.5} /> : <Smartphone size={24} strokeWidth={2.5} />}
          </div>
          <h1 className="text-3xl font-bold text-slate-900">Sign in with {isEmail ? 'Email' : 'WhatsApp'}</h1>
        </div>
        <p className="text-slate-500 font-medium">Enter your details to receive a secure code</p>
      </div>

      <div className="max-w-md w-full bg-white p-8 rounded-3xl shadow-lg border border-slate-100">
        <Link href="/login/method" className="inline-flex items-center text-slate-500 hover:text-slate-800 transition-colors text-sm font-semibold mb-8 group">
        ...
        <form onSubmit={handleSubmit(onSendOTP)} className="space-y-6">
          {isEmail ? (
            ...
            <div>
              <label className="block text-sm font-bold text-slate-800 mb-2">WhatsApp Number</label>
              <Controller
                name="phoneValue"
                control={control}
